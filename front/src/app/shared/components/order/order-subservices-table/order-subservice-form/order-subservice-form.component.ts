...
import {
    ...
    ISubserviceOrder,
    SubserviceOrder
} from '../../../../models/order/SubserviceOrder';
...
export class OrderSubserviceFormComponent {
    ...
    <b>Tem certeza que deseja excluir esse sub-serviço?</b>
    `;
        ...
        this.matDialog.open(ConfirmationComponent, {data: msg})
            ...
            .subscribe((response) => {
                if (response) {
                    this.ordersService.deleteSubservice(this.data.order.id, this.data.subservice.id)
                        .pipe(take(1))
                        .subscribe({
                            next: () => {
